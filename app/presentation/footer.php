		...
		<script type="text/javascript">
			...
			function ajax_update_forum_thread()
			{
				...
				$.post("ajax.php", { 
					...
					function(data) 
					{
						// fadeout, then when complete, change the data and show again
						$("#footer_forum_thread").fadeOut('slow', function() {
							$("#footer_forum_thread").html(data).fadeIn('slow'); 
						});

						ajax_update_forum_thread_timeout = setTimeout("ajax_update_forum_thread()", 10000);
					}				
				);
			}
			...
			// prevent the forum thread reloading on mouseover - code from http://stackoverflow.com/questions/8362698/jquery-stop-animation-on-mouse-over
			$("#footer_forum_thread").hover(function() { 
			    $("#footer_forum_thread").stop();
			    clearTimeout(ajax_update_forum_thread_timeout);       
			}, function( ) {
				ajax_update_forum_thread_timeout = setTimeout("ajax_update_forum_thread()",10000);
			});
